...
func (e *Engine) PostProcess(response *db.Fragment, currentState *state.State) error {
	...
	if err := e.executeManagersInOrder(currentState, func(m managers.Manager) error {
...
func (e *Engine) AddManager(manager managers.Manager) error {
	// Check for duplicates
	for _, m := range e.managers {
		if m.GetID() == manager.GetID() {
			return fmt.Errorf("duplicate manager with ID %s", manager.GetID())
		}
	}

	// Check dependencies
	available := make(map[managers.ManagerID]bool)
	for _, m := range e.managers {
		available[m.GetID()] = true
	}

	for _, dep := range manager.GetDependencies() {
...
func (e *Engine) executeManagersInOrder(currentState *state.State, executeFn func(managers.Manager) error) error {
	// Create a map for quick manager lookup
	managerMap := make(map[managers.ManagerID]managers.Manager)
	for _, m := range e.managers {
	...
	if len(executionOrder) == 0 {
		executionOrder = make([]managers.ManagerID, len(e.managers))
