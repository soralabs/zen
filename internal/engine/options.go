...
func WithManagers(_managers ...managers.Manager) options.Option[Engine] {
	return func(e *Engine) error {
		// Create a map of available managers, checking for duplicates
		available := make(map[managers.ManagerID]managers.Manager)
...
func WithManagerOrder(order []managers.ManagerID) options.Option[Engine] {
	return func(e *Engine) error {
		// Verify all managers in order exist
		managerMap := make(map[managers.ManagerID]bool)
