...
func NewTwitterManager(
	baseOpts []options.Option[managers.BaseManager],
	twitterOpts ...options.Option[TwitterManager],
) (*TwitterManager, error) {
	base, err := managers.NewBaseManager(baseOpts...)
	if err != nil {
		return nil, err
	}
...
func (tm *TwitterManager) GetID() managers.ManagerID {
	return managers.TwitterManagerID
}
...
func (tm *TwitterManager) GetDependencies() []managers.ManagerID {
	return []managers.ManagerID{managers.InsightManagerID, managers.PersonalityManagerID}
}
