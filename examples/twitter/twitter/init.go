...
import (
	...
	twitter_manager "zen/internal/managers/twitter"
	"zen/internal/stores"
	"zen/pkg/id"
	"zen/pkg/logger"
	"zen/pkg/options"
...
func (k *Twitter) create() error {
	...
	insightManager, err := insight.NewInsightManager(
		[]options.Option[managers.BaseManager]{
			managers.WithLogger(k.logger.NewSubLogger("insight", &logger.SubLoggerOpts{})),
			managers.WithContext(k.ctx),
			managers.WithActorStore(actorStore),
			managers.WithLLM(k.llmClient),
			managers.WithSessionStore(sessionStore),
			managers.WithFragmentStore(insightFragmentStore),
			managers.WithInteractionFragmentStore(interactionFragmentStore),
			managers.WithAssistantDetails(assistantName, assistantID),
		},
	)
	if err != nil {
		return err
	...
	personalityManager, err := personality.NewPersonalityManager(
		[]options.Option[managers.BaseManager]{
			managers.WithLogger(k.logger.NewSubLogger("personality", &logger.SubLoggerOpts{})),
			managers.WithContext(k.ctx),
			managers.WithActorStore(actorStore),
			managers.WithLLM(k.llmClient),
			managers.WithSessionStore(sessionStore),
	...
	twitterManager, err := twitter_manager.NewTwitterManager(
		[]options.Option[managers.BaseManager]{
			managers.WithLogger(k.logger.NewSubLogger("twitter", &logger.SubLoggerOpts{})),
			managers.WithContext(k.ctx),
			managers.WithActorStore(actorStore),
			managers.WithLLM(k.llmClient),
			managers.WithSessionStore(sessionStore),
			managers.WithFragmentStore(twitterFragmentStore),
			managers.WithInteractionFragmentStore(interactionFragmentStore),
			managers.WithAssistantDetails(assistantName, assistantID),
		},
